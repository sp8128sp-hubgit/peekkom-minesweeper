...
// Load the custom mine image, but keep the game playable if it fails.
mineImage.onload = function () {
    mineImageLoaded = true;
};
mineImage.onerror = function () {
    mineImageLoaded = false;
};
mineImage.src = "assets/peekkomstar.png";

// Load the lose screen image, with a text fallback if it is unavailable.
loseImage.onload = function () {
    loseImageLoaded = true;
};
loseImage.onerror = function () {
...
function init() {
    ...
    canvas.addEventListener("touchstart", function (evt) {
        ...
        touchTimer = setTimeout(function () {
            touchHandled = true;
            handleCanvasAction(touchStartPos, 2);
        }, 1000);
    }, false);
    ...
    canvas.addEventListener("touchend", function (evt) {
        evt.preventDefault();
        clearTimeout(touchTimer);
        if (!touchHandled && touchStartPos) {
            handleCanvasAction(touchStartPos, 0);
        }
        touchStartPos = null;
    }, false);
...
function handleCanvasAction(mousePos, button) {
...
function drawMine(i, j) {
    ...
    if (mine.isDown && mine.isMine) {
        if (mineImageLoaded) {
            ctx.drawImage(mineImage, x+1, y+1, c-2, c-2);
        } else {
            ctx.beginPath();
            ctx.fillStyle = "black";
            ctx.font = "24px monospace";
            ctx.textAlign = "center";
            ctx.fillText("*", x+(c/2), y+c-7);
            ctx.closePath();
        }
    }
}
...
function drawEndMessage() {
    var panelY = yo + (size.h * c) + 15;
    var panelHeight = 130;
    var panelWidth = size.w * c;
    var centerX = xo + (panelWidth / 2);
    var message = win ? "네가 이겼어! 흑흑" : "네가 졌어!";

    ctx.beginPath();
    ctx.fillStyle = "#C0CfC5";
    ctx.rect(xo, panelY, panelWidth, panelHeight);
    ctx.fill();
    ctx.closePath();
    ...
    ctx.font = "26px sans";
    ctx.textAlign = "center";
    ctx.fillText(message, centerX, panelY + 35);
    ctx.closePath();

    if (loseImageLoaded) {
        ctx.drawImage(loseImage, centerX - 41, panelY + 42, 82, 82);
    } else {
        ctx.beginPath();
        ctx.fillStyle = "black";
        ctx.font = "24px monospace";
        ctx.textAlign = "center";
        ctx.fillText("X_X", centerX, panelY + 90);
        ctx.closePath();
    }
}
